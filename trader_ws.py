...
def write_status():
    ...
    status = {
        ...
        "avg_hold_secs": round(sum(t["hold_secs"] for t in round_trips) / len(round_trips), 1) if round_trips else 0,
        "avg_trip_net": round(sum(t["net"] for t in round_trips) / len(round_trips), 6) if round_trips else 0,
