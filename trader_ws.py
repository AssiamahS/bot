...
# Event logger for command center
...
def on_l2_book(msg: dict):
    ...
    try:
        ...
        if prev_micro > 0:
            ...
            if False:  # fills-only: book changes do NOT trigger reprice
...
def on_order_updates(msg: dict):
    ...
    try:
        ...
        for update in data:
            # DEBUG: log raw payload to diagnose zero-fill bug
            print(f"  [DEBUG orderUpdate] raw: {update}")
            status = update.get("status", "")
            coin = update.get("coin", "")
            side = update.get("side", "")
            sz = float(update.get("sz", 0))
            px = float(update.get("px", 0))
            print(f"  [DEBUG orderUpdate] parsed: status={status} coin={coin} side={side} sz={sz} px={px}")

            if status == "filled":
                # Skip phantom fills with zero size/price (stale position reports)
                if sz == 0 or px == 0:
                    print(f"  [DEBUG] Skipping zero fill: sz={sz} px={px}")
                    continue
                ...
                event_log.log_fill(coin=coin, side="buy" if side == "B" else "sell",
                                   size=sz, price=px, fee=0, closed_pnl=0)
...
def execute_reprice(coin: str, exchange: Exchange, info: Info, address: str, reason: str = ""):
    ...
    if False:  # fills-only: always quote, let the spread config decide
        return
    ...
    # Cap order size to portfolio - max 2x leverage per pair
    portfolio_val = last_balances.get("account_value", 0)
    num_pairs = len(PAIRS) if PAIRS else 1
    max_per_pair = (portfolio_val * 2) / num_pairs if portfolio_val > 0 else ORDER_SIZE_USD
    effective_size_usd = min(ORDER_SIZE_USD, max_per_pair)
    base_size = round(effective_size_usd / mid, s_dec)
    # HL minimum notional is $10 - skip if portfolio too small
    if base_size * mid < 10.0 and portfolio_val < 10.0 * num_pairs:
        print(f"  [{coin}] SKIP: portfolio ${portfolio_val:.2f} too small for min notional")
        return
    ...
    try:
        ...
        for coin, new_pos in new_positions.items():
            ...
            if abs(old_size) > 0 and abs(new_size) < abs(old_size) * 0.1:
                ...
                event_log.log_trip(trip_num=len(round_trips), coin=coin,
                                   net_pnl=round(trip_pnl - trip_fees, 6))
...
def main():
    ...
    time.sleep(3)  # let WS book data arrive
    print("  Placing initial quotes...")
    for pair in PAIRS:
        coin = COIN_MAP.get(pair, pair.replace("-PERP", ""))
        execute_reprice(coin, exchange, info, address, reason="initial")
    print("  Initial quotes placed.")
    event_log.log_start(exchange="hyperliquid", pairs=PAIRS,
                        order_size_usd=ORDER_SIZE_USD, mode="ws_microprice")
            ...
            for pair in PAIRS:
                ...
                if existing and existing.get("ts"):
                    ...
                    if age_ms > STALE_ORDER_MS:
                        pass  # fills-only: no stale repricing
    ...
    pv = last_balances.get("account_value", 0)
    event_log.log_stop(portfolio=round(pv, 2), fills=total_trade_count)
