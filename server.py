...
# VPS sync config
VPS_HOST = "ubuntu@54.234.75.223"
VPS_KEY = os.path.expanduser("~/.ssh/hl-bot-key.pem")
VPS_STATUS_PATH = "~/hyperliquid-sol/trader_status.json"
SYNC_INTERVAL = 30  # seconds between SCP pulls
...
def _sync_vps_status():
    """Background thread: SCP trader_status.json from VPS to local every SYNC_INTERVAL seconds."""
    local_path = STATUS_MAP["/trader_status.json"]
    while True:
        try:
            subprocess.run(
                ["scp", "-i", VPS_KEY, "-o", "ConnectTimeout=5", "-o", "StrictHostKeyChecking=no",
                 f"{VPS_HOST}:{VPS_STATUS_PATH}", local_path],
