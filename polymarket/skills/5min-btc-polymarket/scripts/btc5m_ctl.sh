...
usage() {
  cat <<'EOF'
Usage:
  btc5m_ctl.sh start [--profile conservative|aggressive] [--entry-timeout-min N] [--stake-usd N] [--threshold N] [--poll-sec N] [--close-retry-max N] [--close-retry-delay-sec N] [--dry-run]
  btc5m_ctl.sh status
  btc5m_ctl.sh stop
