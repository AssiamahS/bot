...
| **HIP-3 funding harvester** | RUNNING since 2026-06-09, 26h+ clean uptime. v2.26.3 fixed the two bugs that blocked all order placement: `allMids` missing the `dex` param ("no mid for xyz:SILVER") and SDK `Exchange` built without `perp_dexs` (no asset-id map for xyz). 5 markets: xyz:SILVER/MU/NVDA/AAPL/TSLA, $15–20 each, maker, 600s poll. First real fills expected next time funding crosses the open threshold. |
